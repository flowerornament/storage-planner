...
use crate::core::models::{Decision, DecisionConstraint, DecisionTopology};
...
pub fn run(cmd: DecisionCommands, db: &mut Database, format: OutputFormat) -> Result<()> {
    match cmd {
        ...
        DecisionCommands::Choose {
            ...
            let _ = (&decision, &topology, &rationale);
        ...
        DecisionCommands::Reopen { decision } => {
            let _ = &decision;
            bail!("not yet implemented")
        }
    }
...
fn unconstrain(
    ...
    let constraint: DecisionConstraint = {
        ...
        stmt.query_row(params![decision_id, constraint_type], DecisionConstraint::from_row)
            .map_err(|_| {
                anyhow::anyhow!(
    ...
    match format {
        OutputFormat::Text => {
            println!("Removed constraint: {} from '{}'", constraint_type, decision_title);
        }
        OutputFormat::Json => {
            let json = serde_json::json!({
